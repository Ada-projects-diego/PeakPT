...
export const getExerciseByDateAndId = async (req: Request, res: Response) => {
  try {
    ...
    if (workout) {
      ...
      if (exercise) {
        logger.info('Retrieved exercise by date and ID', { date, exerciseId, exercise });
        res.json(exercise);
      } else {
        logger.info('Exercise not found, returning empty exercise', { date, exerciseId });
        res.json({ id: exerciseId, sets: [] });
      }
    } else {
      logger.info('Workout not found, returning empty exercise', { date, exerciseId });
      res.json({ id: exerciseId, sets: [] });
    }
  } catch (error) {
    logger.error('Error fetching exercise by date and ID', { error, date: req.params.date, exerciseId: req.params.exerciseId });
    res.status(500).json({ message: 'Error fetching exercise' });
  }
};
