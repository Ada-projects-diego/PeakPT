...
// GET a specific exercise for a specific date
app.get('/api/workouts/date/:date/exercises/:exerciseId', async (req, res) => {
  const workouts = await readJsonFile(workoutsPath);
  if (workouts) {
    const workout = workouts.find(w => w.date.startsWith(req.params.date));
    if (workout) {
...
// POST a new set to an exercise for a specific date
app.post('/api/workouts/date/:date/exercises/:exerciseId/sets', async (req, res) => {
  const workouts = await readJsonFile(workoutsPath);
  if (workouts) {
    let workout = workouts.find(w => w.date.startsWith(req.params.date));
    if (!workout) {
    ...
    let exercise = workout.exercises.find(e => e.id === req.params.exerciseId);
    if (!exercise) {
      const exercises = await readJsonFile(exercisesPath);
      const exerciseTemplate = exercises.find(e => e.id === req.params.exerciseId);
      if (!exerciseTemplate) {
